...
const TICKET_CATEGORY_ID = 'ID_CATEGORIE_TICKETS'; // À définir
...
client.on('messageCreate', async message => {
  ...
  if (command === 'help') {
    const embed = new EmbedBuilder()
      ...
      .setDescription(`
**Commandes :**
\`!ticket config\` - Configurer le système de tickets (admin)
...
• Tickets configurables
  ...
  if (command === 'ticket' && args[0] === 'config') {
    ...
    // Envoyer un embed avec les options actuelles
    const embed = new EmbedBuilder()
...
**Image :** ${config.image || 'Aucune'}
**Couleur :** ${config.color}
**Footer :** ${config.footer}
**Options :** ${config.options.join(', ')}
      `)
      .setFooter({ text: 'Utilisez les boutons ci-dessous pour modifier.' });

    const row = new ActionRowBuilder()
      .addComponents(
        new ButtonBuilder().setCustomId('ticket_edit_title').setLabel('Titre').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('ticket_edit_desc').setLabel('Description').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('ticket_edit_image').setLabel('Image').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('ticket_edit_color').setLabel('Couleur').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('ticket_edit_footer').setLabel('Footer').setStyle(ButtonStyle.Primary)
      );
    const row2 = new ActionRowBuilder()
      .addComponents(
        new ButtonBuilder().setCustomId('ticket_edit_options').setLabel('Options (3)').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('ticket_save').setLabel('✅ Sauvegarder').setStyle(ButtonStyle.Success)
      );
    const msg = await message.channel.send({ embeds: [embed], components: [row, row2] });
    // On va stocker temporairement l'ID du message pour la suite (géré via collecteur)
    // Ici simplifié, on utilise un collecteur global
    // On va plutôt utiliser une approche simple : le bot attend les réponses en MP ou dans un salon dédié.
    // Pour simplifier, on va faire une configuration en commande interactive (question/réponse) 
    // Mais c'est complexe. Je propose plutôt une commande qui crée un formulaire via des modals (interactions).
    // Mais comme on utilise des commandes message, on peut faire un menu déroulant.
    // Je vais plutôt faire une commande qui demande chaque champ via des prompts en MP.
    // Pour gagner du temps, je vais implémenter une version simple : !ticket config set <champ> <valeur>
    // et !ticket config show.
  }

  // Version simplifiée de !ticket config
  ...
  if (command === 'giveaway') {
    ...
    // !giveaway #salon durée nb_gagnants titre | description
...
client.on('guildMemberAdd', async member => {
  ...
  // On doit comparer avec les invites stockées avant l'arrivée. On va stocker en mémoire.
  // Pour simplifier, on va juste log le nombre d'invitations sans l'invitateur précis.
  // On peut aussi utiliser le système de cache.
  // Ici on va faire simple : on cherche une invite avec le plus d'usages.
  // Mais c'est approximatif.
  // Je vais implémenter une solution simple : stocker les invites dans un Map.
  // On va initialiser à chaque démarrage.
  if (!client.inviteCache) client.inviteCache = new Map();
  const cached = client.inviteCache.get(member.guild.id) || {};
  // On compte les usages avant/après
  let inviter = null;
  ...
  // Envoyer le message dans le salon d'invites
...
client.on('presenceUpdate', async (oldPresence, newPresence) => {
  ...
  if (!member) return;
  // Vérifier si le membre a le statut contenant le lien
  ...
  if (activity && activity.state) {
    ...
    // Pas de statut personnalisé, retirer le rôle s'il l'a
    const role = member.guild.roles.cache.get(ROLE_STATUT_ID);
...
// À l'initialisation, on vérifie les statuts de tous les membres
client.once('ready', async () => {
  ...
  // Initialiser le cache d'invites pour chaque guild
